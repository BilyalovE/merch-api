# Создаем слой из базового образа (c докерхаба)
FROM mcr.microsoft.com/dotnet/sdk:9.0 AS build

# Устанавливаем внутри контейнера базовую директорию (создаем директорию)
WORKDIR  /src

# Копируем нужные файлы (зависимости проекта) с хоста в рабочую директорию будущего контейнера. / - означает папка, "" - нужно использовать, если в пути есть пробелы
...
# изменили рабочую директорию проекта, теперь нам надо работать из папки, где лежат все файлы для сборки проекта
